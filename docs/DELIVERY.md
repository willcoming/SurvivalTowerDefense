...
自由技能樹工程版已完成，可直接在 [GitHub Pages](https://willcoming.github.io/SurvivalTowerDefense/)、本機或同 Wi-Fi 手機遊玩。規則、流程、本地存檔、畫面與自動化驗證均已交付；下方工程數據引用既有驗證報告，新增的公開站測量另列於 [部署紀錄](GITHUB_PAGES.md)。初版 0.1 交付數據另存於 [歷史報告](history/DELIVERY_INITIAL.md)。
...
`dist/` 是已建置的靜態網站，已透過 `gh-pages` 分支公開發布。Pages 與 localhost 的瀏覽器存檔分開保存。載入最新網站後，新開一局才使用 0.3 自由技能樹；目前未完成的舊局繼續原版本規則，不清除關卡進度或偏好。
