...
import { CHARACTER_MAP, CONTENT_VERSION } from '../src/data/content';
import { DEEP_TREES, DEEP_NODE_MAP } from '../src/data/deep-trees';
const lines=[`# 自由技能樹完整節點表\n\n內容版本 ${CONTENT_VERSION}。所有節點均為本局一次性購入、每項 1 點。以下直接由遊戲內容資料產生。\n\n前置列為「或」時任一父節點即可；終極另外要求本樹已投入 4 點、角色尚無終極且全隊名額未滿。名稱後的「終極」是互斥選擇，普通節點可继续混搭。`];
