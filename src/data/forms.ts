import { SKILL_REWORK_VERSION, usesReworkedSkills } from './reworked-skills';
...
export const PRE_REWORK_VERSION = '0.4.0-dev.2';
export const COLLECTION_CONTENT_VERSION = SKILL_REWORK_VERSION;
export const usesCollection = (s: Pick<RunState, 'contentVersion'>) => usesReworkedSkills(s) || s.contentVersion === PREVIOUS_COLLECTION_VERSION || s.contentVersion === PRE_REWORK_VERSION;
...
const SUMMER: [string,DamageType,string,number,number,number][] = [
  ...
  ['烈日烤宴','plasma','武器爆炸半徑 +25%；直擊傷害 −20%。',.8,1.25,1],
...
export const equippedForm=(s:RunState,id:CharacterId)=>{const f=FORM_MAP[usesCollection(s)?s.config.forms?.[id]??originalForm(id):originalForm(id)];return !usesReworkedSkills(s)&&f.id==='C05-summer'?{...f,damageType:'thermal' as const}:f;};
