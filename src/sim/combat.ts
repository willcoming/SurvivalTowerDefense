...
export function addShield(s:RunState,source:string,value:number,duration:number){
  ...
  const next=Math.min(300-others,Math.max(existing?.value??0,value));
...
export function hitWall(s:RunState,value:number,source:EnemyId){
  let remaining=value;s.shields=s.shields.filter(x=>x.expires>s.tick&&x.value>0).sort((a,b)=>a.expires-b.expires||a.source.localeCompare(b.source));
  for(const shield of s.shields){const v=Math.min(remaining,shield.value);shield.value-=v;remaining-=v;s.stats.shieldAbsorbed+=v;}
...
export function hitEnemy(s:RunState,e:Enemy,p:DamagePacket){
  ...
  const raw=p.raw*(1+(exposure>0?(p.exposureBonus??0):0));
