...
export function createRun(config:RunConfig):RunState{
  ...
  const counters=()=>Object.fromEntries(CHARACTER_IDS.map(id=>[id,0])) as Record<CharacterId,number>;
  const s:RunState={schemaVersion:SCHEMA_VERSION,contentVersion:CONTENT_VERSION,runId:globalThis.crypto?.randomUUID?.()??`run-${Date.now()}-${config.seed}`,config:structuredClone(config),tick:0,phase:'running',pauseReasons:[],wallHp:1000,wallMaxHp:1000,shields:[],xp:0,choicesEarned:0,choicesSpent:0,rerollsRemaining:3,evolvedCount:0,evolutionLimit:config.challengeId==='two-evolutions'?2:3,tacticalReadyAt:0,weapons:config.squadIds.map(id=>({id,branch:null,rank:0,readyAt:0,nextAttack:0,attacks:0,droneAttacks:[0,0],shieldAt:0})),commonRanks:{},preferredBranches:Object.fromEntries(CHARACTER_IDS.map(id=>[id,config.preferredBranches?.[id]??'A'])) as RunState['preferredBranches'],enemies:[],projectiles:[],fields:[],scheduled:[],spawnPlan:[],spawnCursor:0,bossSpawned:false,bossKilled:false,rng:{spawn:seedValue(config.seed,1),draft:seedValue(config.seed,2),visual:seedValue(config.seed,3)},nextEntityId:1,draft:null,nextOfferId:1,events:[],eventSeq:0,actions:[],actionSeq:0,stats:{kills:0,damageByCharacter:counters(),shieldDamageByCharacter:counters(),wallDamageByEnemy:{},shieldAbsorbed:0,controlTicks:counters(),choices:[],casts:[],encountered:[]},outcome:null};
  s.spawnPlan=makeSpawnPlan(s);while(s.spawnCursor<s.spawnPlan.length&&s.spawnPlan[s.spawnCursor].at===0){const p=s.spawnPlan[s.spawnCursor++];createEnemy(s,p.defId,p.x,20,p.xp,p.wave);}return s;
}
...
export function command(s:RunState,cmd:Command):boolean{
  ...
  if(cmd.type==='focus'&&s.draft&&s.config.squadIds.includes(cmd.characterId)){
  ...
  if(cmd.type==='evolution'&&s.draft&&getReadyEvolutions(s).includes(cmd.nodeId)){s.draft.selectedEvolution=cmd.nodeId;rebuildDraft(s);accepted=true;}
...
function stepFields(s:RunState){
  for(const f of s.fields){
    ...
    if(f.kind==='gravity')for(const e of targets){
      ...
      const d=distance(e,f);if(d>1){const amount=Math.min(d,f.pull/30*(boss(e)?.25:1));e.x+=(f.x-e.x)/d*amount;e.y+=(f.y-e.y)/d*amount;if(boss(e))e.moveImmuneUntil=s.tick+ticks(6);if(e.y<450)e.attackAt=0;}
    }
...
export function restoreRun(raw:unknown):RunState{
  ...
  if(!hasNumbers(s,['wallMaxHp','xp','evolvedCount','evolutionLimit','tacticalReadyAt','nextOfferId','eventSeq','actionSeq'])||CHARACTER_IDS.some(id=>!['A','B'].includes(s.preferredBranches?.[id]))||!hasNumbers(s.stats.damageByCharacter,CHARACTER_IDS)||!hasNumbers(s.stats.shieldDamageByCharacter,CHARACTER_IDS)||!hasNumbers(s.stats.controlTicks,CHARACTER_IDS))throw new Error('戰局欄位不完整');
