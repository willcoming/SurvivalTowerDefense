...
function attack(s:RunState,w:WeaponState){
  ...
  if(w.id==='C02'){
    ...
    if(m.chainReturn&&(seen.size>1||usesReworkedSkills(s))&&first.hp>0){emit(s,{kind:'arc',x:target.x,y:target.y,x2:first.x,y2:first.y,source:w.id});hitEnemy(s,first,{...p,raw:p.raw*m.chainReturn,skill:'chain-return'});}
  ...
  if(w.id==='C04'){
    ...
    if(m.fieldDamage){const previous=s.fields.find(f=>f.source===w.id&&f.kind==='gravity'&&!f.ultimate&&f.expires>s.tick),nextTick=previous?.nextTick??s.tick+15;s.fields=s.fields.filter(f=>!(f.source===w.id&&f.kind==='gravity'&&!f.ultimate));
