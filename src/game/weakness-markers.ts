...
export class WeaknessMarkers {
  ...
  update(run:RunState){
    if(!usesCollection(run))return;const visible=run.enemies.filter(e=>run.enemies.length<24||priorityEnemy(e));const ids=new Set(visible.map(e=>e.id));
    for(const [id,image] of this.markers)if(!ids.has(id)){image.destroy();this.markers.delete(id);}
    for(const e of visible){let image=this.markers.get(e.id);if(!image){image=this.scene.add.image(0,0,'weakness-atlas',WEAKNESSES[e.defId]).setDisplaySize(15,15).setDepth(8.6);this.markers.set(e.id,image);}image.setDisplaySize(15,15*this.scene.cameras.main.zoomX/this.scene.cameras.main.zoomY).setPosition(e.x+enemySize(e.defId)/2,e.y-enemySize(e.defId)/2).setVisible(run.bossIntro?.enemyId!==e.id);}
  }
  diagnostics(){return {weaknessMarkers:[...this.markers].filter(([,image])=>image.visible).map(([id])=>id)};}
}
