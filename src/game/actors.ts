...
interface Creature { image: Phaser.GameObjects.Image; defId: EnemyId; hitAt: number }
...
export class CombatActors {
  ...
  private corpse(event: VisualEvent, detail: Detail) {
    ...
    const image = this.scene.add.image(event.x, event.y, key).setDisplaySize(size, size).setDepth(LAYERS.actors + 1);
  ...
  update(run: RunState, delta: number, fresh: VisualEvent[], detail: Detail) {
    ...
    for (const enemy of run.enemies) {
      ...
      if (!creature) {
        ...
        const image = this.scene.add.image(enemy.x, enemy.y, key).setDisplaySize(enemySize(enemy.defId), enemySize(enemy.defId)).setDepth(LAYERS.actors);
        creature = { image, defId: enemy.defId, hitAt: fresh.some(e => e.kind === 'hit' && e.targetId === enemy.id) ? this.clock : -Infinity }; this.creatures.set(enemy.id, creature);
      }
