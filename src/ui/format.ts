...
import { deepUltimate, deepPointCost } from '../sim/deep-tree';
...
export function cardInfo(nodeId: string, run?:RunState) {
  if(DEEP_NODE_MAP[nodeId]){const base=DEEP_NODE_MAP[nodeId],n=resolveSkillNode(base,base.ownerId==='common'?undefined:run?.config.forms?.[base.ownerId]);return {name:n.name,owner:n.ownerId==='common'?'全隊共用':characterName(n.ownerId),effect:n.description,previous:DEEP_TREE_MAP[n.treeId].name,tradeoff:'確認後本局不能洗點。',rank:n.kind==='ultimate'?'終極':n.kind==='entry'?'入口':'模組',icon:n.ownerId==='common'?'':STARTER_IDS.includes(n.ownerId)?assetUrl(`weapons/${n.ownerId}.webp`):formPortrait(originalForm(n.ownerId)),ownerId:n.ownerId==='common'?null:n.ownerId,route:null};}
...
export function weaponLabel(run: RunState, id: CharacterId) { if(usesFreeSkills(run)){const nodes=(run.treeNodes??[]).filter(n=>DEEP_NODE_MAP[n]?.ownerId===id),ult=deepUltimate(run,id);return `${deepPointCost(nodes,run)} 點${ult?` · ${resolveSkillNode(DEEP_NODE_MAP[ult],run.config.forms?.[id]).name}`:' · 可跨樹混搭'}`;} if(usesSkillTrees(run)){const nodes=(run.treeNodes??[]).filter(n=>NODE_MAP[n]?.ownerId===id),ult=ultimateFor(run,id);return `${nodes.length} 節點${ult?` · ${NODE_MAP[ult].name}`:' · 可跨樹混搭'}`;} const w = run.weapons.find(w => w.id === id); return !w?.branch ? '尚未改造' : `${ROUTES.find(r => r.id === `${id}-${w.branch}`)?.name} · ${['', 'I', 'II', 'E'][w.rank]}`; }
