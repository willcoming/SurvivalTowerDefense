...
export function deepGraph(tree:DeepTree,run?:RunState,selected?:string|null){
  ...
  return `<div class="deep-graph" style="height:${height}px" aria-label="${esc(tree.name)}前置路線"><svg class="deep-connections" viewBox="0 0 360 ${height}" preserveAspectRatio="none" aria-hidden="true">${edges}</svg>${tree.nodes.map(n=>{
    const owned=!!run&&deepHas(run,n.id),queued=pending.includes(n.id),reason=run&&!owned&&!queued?deepLock(shadow!,n.id):null,state=owned?'owned':queued?'pending':reason?'locked':'available';
    return `<button class="deep-node ${state} ${n.kind} ${selected===n.id?'inspecting':''}" style="left:${(n.lane*120+60)/3.6}%;top:${n.layer*104+12}px" data-action="${run?'deep-node':'codex-node'}" data-id="${n.id}" data-state="${state}" data-layer="${n.layer}" aria-pressed="${selected===n.id}" aria-label="${esc(n.name)}，${owned?'已取得':queued?'待確認':reason??'1 點可取得'}" ${run?.draft&&state==='locked'?'disabled':''}><span class="node-symbol">${owned?'✓':queued?'●':n.kind==='ultimate'?'✦':reason?'◇':'＋'}</span><strong>${esc(n.name)}</strong><small>${owned?'已取得':queued?'待確認':reason?'查看前置':n.kind==='ultimate'?'終極 · 1 點':'1 技能點'}</small></button>`;
  }).join('')}</div>`;
}
...
export function deepTreePanel(run:RunState,vm:ViewModel){
  ...
  const remaining=run.draft?(run.draft.pointTarget??0)-run.choicesSpent:0,queued=pending.length;
  ...
  return `<div class="modal-backdrop tree-backdrop"><section class="tree-panel deep-panel" data-tree-id="${tree.id}" role="dialog" aria-modal="true" aria-labelledby="deep-title" style="--character:${color}">
    ...
    <div class="tree-intro"><h3>${esc(name)} · ${esc(tree.name)}</h3><p>${esc(tree.purpose)}</p><span>全隊終極 ${run.evolvedCount}/${run.evolutionLimit} · ${remaining?`里程碑 ${run.draft!.choice}/12 · 已選 ${queued}/${remaining}`:'隨時可查看'}</span>${stats?`<p class="weapon-readout">目前傷害 ${stats.damage.toFixed(1)} · 間隔 ${(stats.interval/30).toFixed(2)} 秒 · 射程 ${weaponRange(run,weapon!.id)}</p>`:''}</div>
    ...
    <button class="button primary" data-action="buy-node" ${remaining?(queued!==remaining?'disabled':''): 'disabled'}>${remaining?`確認配置 · ${queued}/${remaining}`:'查看模式 · 經驗升級時可取得'}</button>
