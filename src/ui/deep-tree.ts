...
import { previewDeepNodes, deepHas, deepLock, deepLegalNodes, canSpendDeepPoints, deepNodeCost, deepPointCost } from '../sim/deep-tree';
...
export function deepTreePanel(run:RunState,vm:ViewModel){
  ...
  const base=DEEP_NODE_MAP[view.nodeId??''],node=base?resolveSkillNode(base,form):undefined,pending=run.draft?.pendingNodeIds??[],shadow=previewDeepNodes(run,pending),lock=node&&!pending.includes(node.id)?deepLock(shadow,node.id):null;
  ...
  const confirmAllowed=remaining>0&&queued>0&&(waveAllocation||queued===remaining||(usesSkillNetwork(run)?!canSpendDeepPoints(preview,remaining-queued):!deepLegalNodes(preview).length));
  const weapon=owner==='common'?undefined:run.weapons.find(w=>w.id===owner),stats=weapon?weaponStats(run,weapon):null;
  return `<div class="modal-backdrop tree-backdrop skill-modal-backdrop"><section class="tree-panel deep-panel ${waveAllocation?'wave-allocation':''}" data-tree-id="${tree.id}" role="dialog" aria-modal="true" aria-labelledby="deep-title" style="--character:${color}">
    <header class="tree-header"><div><span class="eyebrow">Ⅱ 戰鬥暫停 · 技能配置</span><h2 id="deep-title">${waveAllocation?`第 ${run.waveFlow!.wave} 波完成`:remaining?'配置技能':'本局技能樹'} <small>已用 ${run.choicesSpent} / 本關 ${operationProfile(run).points} 點</small></h2></div>${remaining?`<span class="points-left" role="status"><span>${waveAllocation?`可用 ${remaining} · 保留 ${remaining-queued}`:'已分配點數'}</span><strong><b>${queued}</b> / ${remaining}</strong></span>`:'<button class="icon-button" data-action="tree-close" aria-label="關閉技能樹">×</button>'}</header>
    ...
    <button class="button primary" data-action="buy-node" ${confirmAllowed?'':'disabled'}>${remaining?(waveAllocation?`確認配置並繼續 · ${queued} 點`:`確認強化 · ${queued}/${remaining} →`):run.waveFlow?'查看模式 · 波末可配置':'查看模式 · 經驗升級時可取得'}</button>
    ${waveAllocation?'<button class="button secondary wave-bank" data-action="bank-wave-points">保留全部點數並繼續</button>':''}
    <div class="deep-footer-note"><small>${waveAllocation?`已選 ${queued} 點，確認後保留 ${remaining-queued} 點。`:remaining?'本次點數用完後自動返回戰鬥。':'關閉後返回原本戰鬥狀態。'}</small><button class="text-button" data-action="tree-save-home">離開關卡 · 本局結束</button></div></footer>
  </section></div>`;
}
