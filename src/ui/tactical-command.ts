import { progressiveExperienceProfile, battleXpAt } from '../data/battle-experience';
...
export function tacticalCommand(save:GameSave,vm:ViewModel) {
  ...
  const boss=ENEMY_MAP[stage.bossId],profile=progressiveExperienceProfile(stageProfile(stage.id,vm.challengeId?'hard':selectedDifficulty(save,stage.id),{challengeId:vm.challengeId,balanceVersion:CURRENT_BALANCE_VERSION}));
