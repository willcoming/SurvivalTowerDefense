...
export function tacticalCommand(save:GameSave,vm:ViewModel) {
  ...
  const content=view==='intel'?`
    ...
    <dl class="tactical-facts"><div><dt>敵群波次</dt><dd>${profile.waves.length} 波</dd></div><div><dt>首領進場</dt><dd>${clock(profile.bossAt*30)}</dd></div><div><dt>作戰期限</dt><dd>不限時</dd></div><div><dt>常規敵人</dt><dd>${profile.enemies} 隻</dd></div><div><dt>技能預算</dt><dd>${profile.points} 點</dd></div><div><dt>終極大招</dt><dd>每個 2 點</dd></div></dl><p class="tactical-caption">清除常規敵群與首領護衛共可取得 ${profile.points*30} 經驗；每 60 經驗獲得 2 點。完整終極路線需 6 點，最多培養 ${Math.min(3,Math.floor(profile.points/6))} 名終極角色，其餘 ${profile.points-Math.min(3,Math.floor(profile.points/6))*6} 點可配置其他技能。共用技能由指揮官等級另計。首領登場帶入 ${profile.escortCount} 隻護衛，共提供 ${profile.escortXp} 經驗，已計入技能預算；持續召喚的敵人不提供經驗。</p>
