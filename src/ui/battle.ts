...
import { battleExperience } from '../sim/experience';
import { CAPTAIN_BONUSES, ultimateForForm, usesReworkedSkills } from '../data/reworked-skills';
import { hasUltimate } from '../sim/ultimates';
import type {CommanderReward} from '../data/commander';
import { hundredCleared, isHundred, runName, type HundredScore } from '../data/hundred';
import { hundredRecord } from './hundred';
import { operationProfile } from '../data/progression';
import { difficultyName, ratingTier } from '../storage/mission-rewards';
import { nextStage } from '../data/campaign';
import { attackType, equippedForm, formPortrait, usesCollection } from '../data/forms';
...
import { EVENT_INFO, VARIANT_INFO } from '../sim/operations';
import { tacticalCooldown } from '../sim/weapons';
import { usesSkillTrees } from '../data/skill-trees';
import { treeUpgradeDialog } from './skill-tree';
import { CHARACTER_MAP, STAGE_MAP, ENEMY_MAP, COMMON_UPGRADES } from '../data/content';
...
import type { BattleSpeed, GameSave, RewardSummary } from '../storage/repository';
...
export function battleShell(run: RunState, speed: BattleSpeed = 1, autoTactical = false) {
  ...
  return `<main class="battle-layout"><aside class="battle-aside"><span class="eyebrow">LIVE OPERATION / ${isHundred(run)?'100 WAVES':run.config.stageId}</span><h1>${esc(runName(run))}</h1><p>守住防線。<br>讓每一份火力，都有意義。</p><div class="mission-objective"><span>MISSION</span><h2>${operationProfile(run).unlimited?'守住防線，清除全部敵人':`${clock(operationProfile(run).deadline*30)} 內擊破首領`}</h2><p>首領於 ${run.waveFlow?(isHundred(run)?'第 100 波':'完成常規波次後'):clock(operationProfile(run).bossAt*30)} 降臨；擊破後清除殘敵。</p></div><div class="live-build" id="live-build"></div><div class="keyboard-hints">${usesReworkedSkills(run)?'終極技自動施放': '<kbd>SPACE</kbd> 隊長技能'} <kbd>ESC</kbd> 暫停</div></aside><section class="battle-phone" aria-label="戰鬥畫面"><header class="battle-hud"><div class="wall-hud"><div><b>防線 <span id="wall-text"></span></b><span id="shield-text"></span></div><div class="hp-track"><i id="wall-bar"></i></div></div><div class="battle-header-controls"><button class="speed-button ${speed > 1 ? 'accelerated' : ''}" id="speed-button" data-action="speed" aria-label="戰鬥速度 ${speed} 倍，點擊切換至 ${speed === 3 ? 1 : speed + 1} 倍" title="切換戰鬥速度：1× → 2× → 3×">${speed}×</button><button class="icon-button" data-action="pause" aria-label="暫停戰鬥">Ⅱ</button></div><div class="time-hud"><span id="wave-text"></span><strong id="time-text"></strong><span id="boss-countdown"></span><span id="evolution-text"></span></div><div class="xp-track"><i id="xp-bar"></i></div><div class="xp-caption"><span>共鳴經驗</span><span id="xp-text"></span></div>${usesFreeSkills(run)?'<div id="operation-event" class="operation-event" role="status"></div>':''}</header><div id="battle-canvas" class="battle-canvas" role="img" aria-label="敵人從上方進攻，五人小隊在底部自動防守"></div><div id="weapon-strip" class="weapon-strip"></div><div id="mechanic-readout" class="mechanic-readout" aria-live="off"></div><div class="range-toolbar"><span id="range-info" role="status">點選角色查看射程</span><button data-action="view-build" aria-label="查看本局構築">構築</button></div>${usesReworkedSkills(run)?`<div class="ultimate-strip" aria-label="自動終極技">${run.weapons.map(w=>`<div title="${esc(ultimateForForm(w.id,run.config.forms?.[w.id]).name)}"><small>${esc(CHARACTER_MAP[w.id].name)}</small><b id="ultimate-${w.id}">未取得</b></div>`).join('')}</div><div class="ultimate-cue" id="ultimate-cue" role="status"></div>`:`<div class="tactical-control"><button class="tactical-button" data-action="cast" id="tactical-button"><span class="tactical-icon">✦</span><span><small>隊長 · ${esc(captain.name)}</small><b>${esc(captain.tacticalName)}</b></span><strong id="tactical-status">就緒</strong></button><button class="auto-tactical-button" id="auto-tactical-button" data-action="auto-tactical" aria-label="自動施放隊長技能" aria-pressed="${autoTactical}" ${run.config.challengeId === 'no-skill' ? 'disabled' : ''}><span>自動施放</span><b id="auto-tactical-status">${autoTactical ? '已開啟' : '已關閉'}</b></button></div>`}</section><aside class="battle-right"><span class="eyebrow">FIELD COMMANDER</span><div class="battle-captain">${portrait(captain.id)}<div><strong>${esc(captain.name)}</strong><span>${esc(captain.english)}</span></div></div><p>${esc(usesReworkedSkills(run)?CAPTAIN_BONUSES[captain.id].description:captain.tacticalDescription)}</p><div class="tactical-note"><span>TACTICAL NOTE</span><p id="threat-note">武器自動攻擊。配置控場分支，反制敵人的蓄力。</p></div><p class="battle-autosave">本機自動保存<br><small>切換到背景時會暫停。</small></p></aside></main><div id="battle-overlay"></div>`;
}
export function buildMarkup(run: RunState) {
  return `<h2>本局構築 <span>${run.choicesSpent} / ${usesFreeSkills(run)?operationProfile(run).points:18}</span></h2>${run.weapons.map(w => `<div class="build-line"><span style="background:${CHARACTER_MAP[w.id].color}"></span><b>${esc(CHARACTER_MAP[w.id].name)}</b><small>${esc(weaponLabel(run, w.id))}</small></div>`).join('')}${usesFreeSkills(run)?`<p class="common-build">${run.commanderSkillVersion===1?`指揮官共用技能 · ${run.config.commanderNodes?.length??0} 項（出擊前設定）`:`共用技能 · ${(run.treeNodes??[]).filter(n=>DEEP_NODE_MAP[n]?.ownerId==='common').length} 點`}</p>`:''}${COMMON_UPGRADES.filter(c => run.commonRanks[c.id]).map(c => `<p class="common-build">${esc(c.name)} × ${run.commonRanks[c.id]}</p>`).join('')}`;
}
interface HudCache {
...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  const weaponKey = run.weapons.map(w => `${w.id}:${w.branch}:${w.rank}:${run.treeNodes?.join(',')??''}`).join(',');
  ...
  set('boss-countdown', operationProfile(run).unlimited ? (run.bossKilled ? '清除殘敵' : run.bossSpawned ? '首領降臨' : run.waveFlow?(isHundred(run)?'首領：第 100 波':'首領：常規波次後'):`首領倒數 ${clock(Math.max(0,operationProfile(run).bossAt*30-run.tick))}`) : ''); set('evolution-text', `${isHundred(run)?`已完成 ${hundredCleared(run)} 波 · `:''}${usesFreeSkills(run)?'終極':'進化'} ${run.evolvedCount}/${run.evolutionLimit}`);
  const experience=battleExperience(run),{maximum,required:threshold}=experience;
  set('shell-skill-points', `${run.choicesSpent} / ${maximum}`);
  width('xp-bar', `${experience.current/threshold*100}%`);
  set('xp-text',run.waveFlow?`Lv.${experience.level} · ${experience.capped?'經驗已滿':`${experience.current} / ${threshold}`} · 待配 ${run.choicesEarned-run.choicesSpent} 點 · 波末配置`:run.choicesEarned>=maximum?'技能點已全數取得':usesFreeSkills(run)?`${run.xp%60} / 60 · ${run.choicesEarned/2}/${maximum/2} 次 · ${run.choicesSpent}/${maximum} 點`:`${run.xp%40} / 40 · ${run.choicesSpent}/18 次`);
  if(usesFreeSkills(run)){const wave=run.wavePlan?.find(w=>w.wave===currentWave(run));set('operation-event',wave?`${VARIANT_INFO[wave.variant].name} · ${EVENT_INFO[wave.event].name}`:'核心決戰 · 查看構築可查首領弱點');}
  if(usesReworkedSkills(run)) {
    for(const w of run.weapons){const remaining=Math.max(0,Math.ceil(((w.ultimateReadyAt??0)-run.tick)/30));set(`ultimate-${w.id}`,hasUltimate(run,w.id)?remaining?`${remaining}s`:'等待觸發':'未取得');}
    const latest=run.stats.ultimateCasts?.at(-1);set('ultimate-cue',latest&&run.tick-latest.tick<75?`${CHARACTER_MAP[latest.ownerId].name} · ${ultimateForForm(latest.ownerId,latest.formId).name}`:`隊長加成 · ${CAPTAIN_BONUSES[run.config.captainId].name}`);
  }
  const cooldown = Math.max(0, Math.ceil((run.tacticalReadyAt - run.tick) / 30));
  const noTarget = run.enemies.length === 0 && run.config.captainId !== 'C06' || run.config.captainId==='C07'&&!run.mines?.length;
  ...
  const fill = `${Math.min(100, cooldown / (tacticalCooldown(run)/30) * 100)}%`;
  ...
  set('threat-note', charge ? `⚠ ${ENEMY_MAP[charge.defId].name} 蓄力中。${immunity || '可使用有效暈眩或位移打斷'}。` : run.bossKilled ? '核心已摧毀，清除剩餘敵人即可完成行動。' : run.bossSpawned ? '配置破盾、控場與曝露增傷，反制首領。' : '武器與已取得的終極技自動施放。透過技能分支調整反制能力。');
  ...
  if (cache.stripKey !== weaponKey) {
    ...
    if (strip) {strip.innerHTML = run.weapons.map(w => `<button id="range-${w.id}" data-action="view-range" data-id="${w.id}" aria-pressed="false" aria-label="查看${esc(CHARACTER_MAP[w.id].name)}的射程"><span style="--character:${CHARACTER_MAP[w.id].color}">${portrait(w.id).replace(formPortrait(`${w.id}-original`),formPortrait(equippedForm(run,w.id).id))}</span><b>${esc(CHARACTER_MAP[w.id].name)}</b><small>${usesRangeRules(run) ? RANGE_LABEL[w.id] : '全域'}·${usesSkillTrees(run)?(w.rank===3?'終極':'改造'):['基礎', 'I', 'II', 'E'][w.rank]}</small>${usesCollection(run)?elementBadge(attackType(run,w.id)):' '}</button>`).join('');strip.querySelectorAll<HTMLImageElement>('img').forEach(img=>{img.addEventListener('load',()=>keyInterfaceImage(img),{once:true});if(img.complete)keyInterfaceImage(img);});}
  }
  ...
  const mechanics=run.weapons.filter(w=>w.id==='C07'||w.id==='C08').map(w=>w.id==='C07'?`汐音・已部署 ${run.mines?.length??0} 枚地雷`:`熾夏・熱量 ${Math.ceil(w.heat??0)}% ${w.cooling?'散熱停火':(w.ventUntil??0)>run.tick?'排熱加速':'射擊待命'}`).join(' ｜ ');set('mechanic-readout',mechanics);
}

export function upgradeDialog(run: RunState, vm: ViewModel) {
  const offer = run.draft; if (!offer) return '';
  if(usesSkillTrees(run))return treeUpgradeDialog(run,vm);
...
export function pauseDialog(run: RunState, save: GameSave, vm: ViewModel) {
  return `<div class="modal-backdrop"><section class="dialog pause-dialog" role="dialog" aria-modal="true" aria-labelledby="pause-title"><span class="eyebrow">TAKE A BREATH</span><h2 id="pause-title">${run.pauseReasons.includes('orientation') ? '請將手機轉回直向' : run.pauseReasons.includes('error') ? '紀錄需要妳的注意' : '防線，等妳回來。'}</h2><p>${esc(runName(run))} · ${clock(run.tick)} · 防線 ${num(run.wallHp)}</p>${run.pauseReasons.includes('error') ? '<p>本局已安全暫停。請先重試儲存或讀取最新紀錄，再繼續行動。</p><button class="button secondary" data-action="save-retry">重試儲存</button><button class="button secondary" data-action="reload">讀取最新紀錄</button>' : ''}<button class="button primary" data-action="resume" ${run.pauseReasons.includes('orientation') || run.pauseReasons.includes('error') ? 'disabled' : ''}>返回戰鬥 →</button><button class="button secondary" data-action="view-build">${vm.showBuild ? '收合目前構築' : '查看目前構築'}</button>${vm.showBuild ? `<div class="inline-build">${buildMarkup(run)}</div>` : ''}${settings(save, vm.saveStatus, true)}<button class="button secondary" data-action="save-home">離開關卡 · 本局結束</button><button class="text-button danger-text" data-action="abandon-confirm">放棄本次行動</button></section></div>`;
}
export function tutorialDialog(run?: RunState) {
  return `<div class="modal-backdrop"><section class="dialog tutorial-dialog" role="dialog" aria-modal="true" aria-labelledby="tutorial-title"><span class="eyebrow">FIRST DEPLOYMENT</span><h2 id="tutorial-title">指揮官，準備好了嗎？</h2><ol class="tutorial-steps"><li><b>01</b><span><strong>守住共同防線</strong>小隊自動索敵射擊。防線歸零，行動失敗。</span></li><li><b>02</b><span><strong>把火力，改造成妳的答案</strong>${run?.waveFlow?`${run.experienceVersion===2?'首次升級需 30 經驗，之後每級增加 5 經驗；每升 1 級獲得 1 點':`每 ${battleExperience(run).required} 經驗升 1 級、獲得 ${battleExperience(run).pointsPerLevel} 點`}，清完本波後統一配置。可部分使用或保留點數，確認後開始下一波。`:run&&usesFreeSkills(run)?'每 60 經驗獲得 2 點。直接在技能樹選擇，兩點用完後繼續。':`共鳴經驗達標後三選一；${run&&usesSkillTrees(run)?'自選一格、隨機兩格。可跨樹混搭，每人一個終極、全隊最多三個。':'每人 I → II → E，全隊最多三把 E。'}`}</span></li><li><b>03</b><span><strong>${run&&usesReworkedSkills(run)?'隊長加成與自動終極技':'在關鍵時刻下令'}</strong>${run&&usesReworkedSkills(run)?'隊長提供開場生效的隊伍加成；分支終極取得後先完成冷卻，再依目標自動施放。':run&&usesFreeSkills(run)?'隊長技能開局先冷卻，就緒後可手動或自動施放。':'隊長技能就緒時可手動或自動施放。'}敵人蓄力時可用控場打斷。</span></li><li><b>04</b><span><strong>換個搭配，立即重試</strong>不用農資源。失敗可保留相同戰局，重新分配火力。</span></li></ol><button class="button primary" data-action="tutorial-done">明白，開始防守 →</button><button class="text-button" data-action="tutorial-done">跳過引導</button></section></div>`;
}
export function result(run: RunState, rewards?:RewardSummary,commanderReward?:CommanderReward,best?:HundredScore) {
  ...
  const next = isHundred(run)?undefined:nextStage(run.config.stageId);
  return `<main class="content-screen result-screen"><section class="result-hero ${won ? 'victory' : 'defeat'}"><span class="eyebrow">${won ? 'OPERATION COMPLETE' : 'REGROUP & RETURN'}</span><h1>${won ? '黎明，向前一步。' : run.outcome === 'abandoned' ? '撤退，也是下一步的開始。' : '這次的答案，還能更好。'}</h1><p>${esc(runName(run))} · ${isHundred(run)?'100 波':difficultyName(run.config.difficulty)}${won&&!isHundred(run)?` · ${ratingTier(run)} 級評價`:""} · ${won ? '防線奪回成功' : run.outcome === 'timeout' ? '行動時間已到' : run.outcome === 'abandoned' ? '行動已放棄' : '防線失守'}</p><div class="result-numbers"><div><strong>${clock(run.tick)}</strong><span>有效戰鬥時間</span></div><div><strong>${num(run.wallHp)}</strong><span>防線剩餘 / ${run.wallMaxHp}</span></div><div><strong>${run.evolvedCount}</strong><span>完成終極進化</span></div></div></section>
  ${isHundred(run)?`<section class="hundred-record"><h2>本次完成 ${hundredCleared(run)} / 100 波</h2><p>擊殺 ${run.stats.kills} · ${best?.runId===run.runId?'本局創下最高紀錄':'繼續突破最高紀錄'}</p></section>${hundredRecord(best)}`:''}
  ${commanderReward?`<section class="commander-result" aria-label="指揮官成長結算"><h2>指揮官經驗 ＋${commanderReward.xp}</h2><p>${commanderReward.points?`Lv.${commanderReward.beforeLevel} → Lv.${commanderReward.afterLevel} · 獲得 ${commanderReward.points} 點共用技能點`:`目前 Lv.${commanderReward.afterLevel}`}${commanderReward.firstClear?' · 含首次通關加成':''}</p><button class="button secondary" data-action="commander">設定共用技能 →</button></section>`:''}${isHundred(run)?'':`<section class="result-rewards" aria-label="本次招募獎勵"><span class="eyebrow">RECRUITMENT REWARDS</span><h2>${rewards&&(rewards.tickets||rewards.points)?`首通獎勵 · ${rewards.tickets?`招募券 +${rewards.tickets}`:""} ${rewards.points?`共鳴點數 +${rewards.points}`:""}`:"本次沒有新增招募獎勵"}</h2><p>${rewards?.completed?`51 個目標全部完成，已補齊剩餘 ${rewards.forms} 個形態！`:won?"重複通關不再發獎。完成新的關卡或挑戰，繼續收集。":"失敗或放棄不發獎，也不扣除招募資源。"}</p><button class="button secondary" data-action="recruitment">前往星際招募 →</button></section>`}
  ${won&&!isHundred(run)?`<section class="result-story"><h2>行動後記</h2>${STAGE_MAP[run.config.stageId].outro.map(line=>`<p>${esc(line)}</p>`).join("")}</section>`:""}
