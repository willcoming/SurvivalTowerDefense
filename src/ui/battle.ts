...
import { attackType, ELEMENTS, equippedForm, formPortrait, usesCollection } from '../data/forms';
...
export function buildMarkup(run: RunState) {
  return `<h2>本局構築 <span>${run.choicesSpent} / ${usesFreeSkills(run)?24:18}</span></h2>${run.weapons.map(w => `<div class="build-line"><span style="background:${CHARACTER_MAP[w.id].color}"></span><b>${esc(CHARACTER_MAP[w.id].name)}</b><small>${esc(weaponLabel(run, w.id))}</small></div>`).join('')}${usesFreeSkills(run)?`<p class="common-build">共用技能 · ${(run.treeNodes??[]).filter(n=>DEEP_NODE_MAP[n]?.ownerId==='common').length} 點</p>`:''}${COMMON_UPGRADES.filter(c => run.commonRanks[c.id]).map(c => `<p class="common-build">${esc(c.name)} × ${run.commonRanks[c.id]}</p>`).join('')}`;
}
interface HudCache {
...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('time-text', clock(480 * 30 - run.tick)); set('evolution-text', `${usesFreeSkills(run)?'終極':'進化'} ${run.evolvedCount}/${run.evolutionLimit}`);
  const maximum=usesFreeSkills(run)?24:18,threshold=usesFreeSkills(run)?60:40;
  width('xp-bar', `${run.choicesEarned>=maximum?100:run.xp%threshold/threshold*100}%`);
  set('xp-text',run.choicesEarned>=maximum?'技能點已全數取得':usesFreeSkills(run)?`${run.xp%60} / 60 · ${run.choicesEarned/2}/12 次 · ${run.choicesSpent}/24 點`:`${run.xp%40} / 40 · ${run.choicesSpent}/18 次`);
  if(usesFreeSkills(run)){const wave=run.wavePlan?.find(w=>w.wave===Math.floor(run.tick/1350)+1);set('operation-event',wave?`${VARIANT_INFO[wave.variant].name} · ${EVENT_INFO[wave.event].name}`:'核心決戰 · 查看構築可查首領弱點');}
...
export function result(run: RunState, rewards?:RewardSummary) {
  ...
  <section class="result-rewards" aria-label="本次招募獎勵"><span class="eyebrow">RECRUITMENT REWARDS</span><h2>${rewards&&(rewards.tickets||rewards.points)?`首通獎勵 · ${rewards.tickets?`招募券 +${rewards.tickets}`:""} ${rewards.points?`招募點數 +${rewards.points}`:""}`:"本次沒有新增招募獎勵"}</h2><p>${rewards?.completed?`51 個目標全部完成，已補齊剩餘 ${rewards.forms} 個形態！`:won?"重複通關不再發獎。完成新的關卡或挑戰，繼續收集。":"失敗或放棄不發獎，也不扣除招募資源。"}</p><button class="button secondary" data-action="recruitment">前往星際招募 →</button></section>
