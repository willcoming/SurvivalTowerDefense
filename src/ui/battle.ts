...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('threat-note', charge ? `⚠ ${ENEMY_MAP[charge.defId].name} 蓄力中。${immunity || '可使用有效暈眩或位移打斷'}。` : run.bossKilled ? '核心已摧毀，清除剩餘敵人即可完成行動。' : run.tick > operationProfile(run).bossAt * 30 ? '觀察首領的蓄力與曝露窗口，分配技能。' : '攻擊自動進行。預留技能，回應敵人的蓄力。');
...
export function tutorialDialog(run?: RunState) {
  return `<div class="modal-backdrop"><section class="dialog tutorial-dialog" role="dialog" aria-modal="true" aria-labelledby="tutorial-title"><span class="eyebrow">FIRST DEPLOYMENT</span><h2 id="tutorial-title">指揮官，準備好了嗎？</h2><ol class="tutorial-steps"><li><b>01</b><span><strong>守住共同防線</strong>小隊自動索敵射擊。防線歸零，行動失敗。</span></li><li><b>02</b><span><strong>把火力，改造成妳的答案</strong>${run&&usesFreeSkills(run)?'每 60 經驗獲得 2 點。直接在技能樹選擇，兩點用完後繼續。':`共鳴經驗達標後三選一；${run&&usesSkillTrees(run)?'自選一格、隨機兩格。可跨樹混搭，每人一個終極、全隊最多三個。':'每人 I → II → E，全隊最多三把 E。'}`}</span></li><li><b>03</b><span><strong>在關鍵時刻下令</strong>${run&&usesFreeSkills(run)?'隊長技能開局先冷卻，就緒後可手動或自動施放。':'隊長技能就緒時可手動或自動施放。'}敵人蓄力時可用控場打斷。</span></li><li><b>04</b><span><strong>換個搭配，立即重試</strong>不用農資源。失敗可保留相同戰局，重新分配火力。</span></li></ol><button class="button primary" data-action="tutorial-done">明白，開始防守 →</button><button class="text-button" data-action="tutorial-done">跳過引導</button></section></div>`;
}
