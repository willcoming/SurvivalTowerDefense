...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('boss-countdown', operationProfile(run).unlimited ? (run.bossKilled ? '清除殘敵' : run.bossSpawned ? '首領降臨' : `首領倒數 ${clock(Math.max(0,operationProfile(run).bossAt*30-run.tick))}`) : ''); set('evolution-text', `${usesFreeSkills(run)?'終極':'進化'} ${run.evolvedCount}/${run.evolutionLimit}`);
...
export function result(run: RunState, rewards?:RewardSummary,commanderReward?:CommanderReward) {
