...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('wave-text', run.tick >= operationProfile(run).bossAt * 30 ? '首領降臨' : `WAVE ${Math.min(operationProfile(run).waves.length, Math.floor(run.tick / (operationProfile(run).interval * 30)) + 1)} / ${operationProfile(run).waves.length}`);
  set('time-text', clock(operationProfile(run).deadline * 30 - run.tick)); set('evolution-text', `${usesFreeSkills(run)?'終極':'進化'} ${run.evolvedCount}/${run.evolutionLimit}`);
