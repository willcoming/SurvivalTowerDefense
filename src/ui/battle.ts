...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('xp-text',run.waveFlow?`${run.choicesEarned>=maximum?'經驗已滿':`${run.xp%60} / 60`} · 待配 ${run.choicesEarned-run.choicesSpent} 點 · 波末配置`:run.choicesEarned>=maximum?'技能點已全數取得':usesFreeSkills(run)?`${run.xp%60} / 60 · ${run.choicesEarned/2}/${maximum/2} 次 · ${run.choicesSpent}/${maximum} 點`:`${run.xp%40} / 40 · ${run.choicesSpent}/18 次`);
...
export function tutorialDialog(run?: RunState) {
  return `<div class="modal-backdrop"><section class="dialog tutorial-dialog" role="dialog" aria-modal="true" aria-labelledby="tutorial-title"><span class="eyebrow">FIRST DEPLOYMENT</span><h2 id="tutorial-title">指揮官，準備好了嗎？</h2><ol class="tutorial-steps"><li><b>01</b><span><strong>守住共同防線</strong>小隊自動索敵射擊。防線歸零，行動失敗。</span></li><li><b>02</b><span><strong>把火力，改造成妳的答案</strong>${run?.waveFlow?'每 60 經驗獲得 2 點，清完本波後統一配置。可部分使用或保留點數，確認後開始下一波。':run&&usesFreeSkills(run)?'每 60 經驗獲得 2 點。直接在技能樹選擇，兩點用完後繼續。':`共鳴經驗達標後三選一；${run&&usesSkillTrees(run)?'自選一格、隨機兩格。可跨樹混搭，每人一個終極、全隊最多三個。':'每人 I → II → E，全隊最多三把 E。'}`}</span></li><li><b>03</b><span><strong>${run&&usesReworkedSkills(run)?'隊長加成與自動終極技':'在關鍵時刻下令'}</strong>${run&&usesReworkedSkills(run)?'隊長提供開場生效的隊伍加成；分支終極取得後先完成冷卻，再依目標自動施放。':run&&usesFreeSkills(run)?'隊長技能開局先冷卻，就緒後可手動或自動施放。':'隊長技能就緒時可手動或自動施放。'}敵人蓄力時可用控場打斷。</span></li><li><b>04</b><span><strong>換個搭配，立即重試</strong>不用農資源。失敗可保留相同戰局，重新分配火力。</span></li></ol><button class="button primary" data-action="tutorial-done">明白，開始防守 →</button><button class="text-button" data-action="tutorial-done">跳過引導</button></section></div>`;
}
