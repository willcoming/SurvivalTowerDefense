...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  if (cache.stripKey !== weaponKey) {
    ...
    if (strip) strip.innerHTML = run.weapons.map(w => `<button id="range-${w.id}" data-action="view-range" data-id="${w.id}" aria-pressed="false" aria-label="查看${esc(CHARACTER_MAP[w.id].name)}的射程"><span style="--character:${CHARACTER_MAP[w.id].color}">${portrait(w.id)}</span><b>${esc(CHARACTER_MAP[w.id].name)}</b><small>${usesRangeRules(run) ? RANGE_LABEL[w.id] : '全域'}·${usesSkillTrees(run)?(w.rank===3?'終極':'改造'):['基礎', 'I', 'II', 'E'][w.rank]}</small></button>`).join('');
  }
