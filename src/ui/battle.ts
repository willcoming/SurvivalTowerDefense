...
export function updateHud(run: RunState, speed: BattleSpeed = 1, autoTactical = false, selectedRange: CharacterId | null = null) {
  ...
  set('wave-text', !operationProfile(run).unlimited && run.tick >= operationProfile(run).bossAt * 30 ? '首領降臨' : `WAVE ${Math.min(operationProfile(run).waves.length, Math.floor(run.tick / (operationProfile(run).interval * 30)) + 1)} / ${operationProfile(run).waves.length}`);
  set('time-text', operationProfile(run).unlimited ? `已戰鬥 ${clock(run.tick)}` : clock(operationProfile(run).deadline * 30 - run.tick));
  set('boss-countdown', operationProfile(run).unlimited ? (run.bossKilled ? '清除殘敵' : run.bossSpawned ? '首領降臨' : `首領倒數 ${clock(Math.max(0,operationProfile(run).bossAt*30-run.tick))}`) : ''); set('evolution-text', `${isHundred(run)?`已完成 ${hundredCleared(run)} 波 · `:''}${usesFreeSkills(run)?'終極':'進化'} ${run.evolvedCount}/${run.evolutionLimit}`);
  const maximum=usesFreeSkills(run)?operationProfile(run).points:18,threshold=usesFreeSkills(run)?60:40;
  set('shell-skill-points', `${run.choicesSpent} / ${maximum}`);
  width('xp-bar', `${run.choicesEarned>=maximum?100:run.xp%threshold/threshold*100}%`);
  set('xp-text',run.choicesEarned>=maximum?'技能點已全數取得':usesFreeSkills(run)?`${run.xp%60} / 60 · ${run.choicesEarned/2}/${maximum/2} 次 · ${run.choicesSpent}/${maximum} 點`:`${run.xp%40} / 40 · ${run.choicesSpent}/18 次`);
  if(usesFreeSkills(run)){const wave=run.wavePlan?.find(w=>w.wave===Math.floor(run.tick/(operationProfile(run).interval*30))+1);set('operation-event',wave?`${VARIANT_INFO[wave.variant].name} · ${EVENT_INFO[wave.event].name}`:'核心決戰 · 查看構築可查首領弱點');}
  ...
  set('threat-note', charge ? `⚠ ${ENEMY_MAP[charge.defId].name} 蓄力中。${immunity || '可使用有效暈眩或位移打斷'}。` : run.bossKilled ? '核心已摧毀，清除剩餘敵人即可完成行動。' : run.tick > operationProfile(run).bossAt * 30 ? '配置破盾、控場與曝露增傷，反制首領。' : '武器與已取得的終極技自動施放。透過技能分支調整反制能力。');
...
export function tutorialDialog(run?: RunState) {
  return `<div class="modal-backdrop"><section class="dialog tutorial-dialog" role="dialog" aria-modal="true" aria-labelledby="tutorial-title"><span class="eyebrow">FIRST DEPLOYMENT</span><h2 id="tutorial-title">指揮官，準備好了嗎？</h2><ol class="tutorial-steps"><li><b>01</b><span><strong>守住共同防線</strong>小隊自動索敵射擊。防線歸零，行動失敗。</span></li><li><b>02</b><span><strong>把火力，改造成妳的答案</strong>${run&&usesFreeSkills(run)?'每 60 經驗獲得 2 點。直接在技能樹選擇，兩點用完後繼續。':`共鳴經驗達標後三選一；${run&&usesSkillTrees(run)?'自選一格、隨機兩格。可跨樹混搭，每人一個終極、全隊最多三個。':'每人 I → II → E，全隊最多三把 E。'}`}</span></li><li><b>03</b><span><strong>${run&&usesReworkedSkills(run)?'隊長加成與自動終極技':'在關鍵時刻下令'}</strong>${run&&usesReworkedSkills(run)?'隊長提供開場生效的隊伍加成；分支終極取得後先完成冷卻，再依目標自動施放。':run&&usesFreeSkills(run)?'隊長技能開局先冷卻，就緒後可手動或自動施放。':'隊長技能就緒時可手動或自動施放。'}敵人蓄力時可用控場打斷。</span></li><li><b>04</b><span><strong>換個搭配，立即重試</strong>不用農資源。失敗可保留相同戰局，重新分配火力。</span></li></ol><button class="button primary" data-action="tutorial-done">明白，開始防守 →</button><button class="text-button" data-action="tutorial-done">跳過引導</button></section></div>`;
}
