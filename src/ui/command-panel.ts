...
export function commandPanel(save: GameSave, vm: ViewModel, selectedRange: string | null) {
  ...
  const profile=run?operationProfile(run):stageProfile(stage.id,vm.challengeId?'hard':selectedDifficulty(save,stage.id),{challengeId:vm.challengeId});
  ...
  const content = kind === 'challenges' ? challengeChoices(save,vm) : kind === 'recruit-rules' ? recruitmentRules() : kind === 'objectives' ? operationObjectives(save, stage.id) : kind === 'shortcuts' ? `<div class="command-menu-grid">${commandButtons(!!run, vm.page === 'home' ? 'left' : undefined)}</div>${vm.page === 'home' ? '<button class="button secondary" data-action="intel">作戰說明與挑戰設定 →</button>' : ''}` : kind === 'intel' ? `
    ...
    <p>${esc(stage.description)}</p><div class="command-metrics"><span><small>敵群</small><b>${profile.waves.length} 波</b></span><span><small>首領</small><b>${clock(profile.bossAt*30)}</b></span><span><small>期限</small><b>${profile.unlimited?'不限時':clock(profile.deadline*30)}</b></span><span><small>常規敵人</small><b>${profile.enemies} 隻</b></span><span><small>技能預算</small><b>${profile.points} 點</b></span></div>
    <h3>優先目標 · ${esc(boss.name)}</h3><p>${esc(boss.mechanic)}</p><p class="command-counter">${esc(boss.counter)}</p>` : kind === 'waves' ? `
    <p>${run ? '接下來的敵群與戰場事件' : '預定波次與威脅組合；按敵情調整技能配置。'}</p><div class="wave-preview-list">${run ? nextIntel(run).map(row => `<article><span>${clock(row.at)}</span><div><h3>第 ${row.wave} 波 · ${esc(row.name??'敵群推進')} · ${VARIANT_INFO[row.brief?.variant ?? 'standard'].name}</h3><p>${row.counts.map(([id, count]) => `${esc(ENEMY_MAP[id].name)} ×${count}`).join('、') || '本波已部署'}</p><small>${EVENT_INFO[row.brief?.event ?? 'none'].description}</small>${row.hint?`<p class="command-counter">${esc(row.hint)}</p>`:''}</div></article>`).join('') || '<p>所有常規波次已進場，準備迎戰首領。</p>' : profile.waves.map((wave, i) => `<article><span>${clock(i * profile.interval*30)}</span><div><h3>第 ${i + 1} 波 · ${esc(profile.waveNames?.[i]??'敵群推進')} · ${profile.waveXp[i]} 經驗</h3><p>${wave.split(' ').map(token => { return `${esc(ENEMY_MAP[ENEMY_CODE[token[0]]].name)} ×${token.slice(1)}`; }).join('、')}</p><small>${esc(profile.waveHints?.[i]??'')}</small></div></article>`).join('')}</div><h3>敵方情報與弱點</h3>${enemyContent}` : kind === 'auto' ? `
