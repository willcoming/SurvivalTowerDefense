...
export function rosterDialog(save: GameSave, panel: RosterPanel, busy: boolean, temporary: boolean, max = 5, message = '', editing = false): string {
  ...
  return `<div class="modal-backdrop roster-backdrop"><section class="dialog roster-dialog" role="dialog" aria-modal="true" aria-label="${esc(title)}" data-roster-view="${panel.view}" style="--character:${character.color}">
    ...
    <div class="roster-panel-body">${wardrobe ? `
      ...
      ${!owned ? `<section class="wardrobe-acquisition"><h3>▣ 取得方式</h3><p>${STARTER_FORMS.includes(previewId) ? '初始隊員的原始形態會隨新進度解鎖。' : '星際招募可取得此造型，也可使用 100 共鳴碎片指定兌換。完成全部 51 項獎勵目標可補齊全收集。'}</p><button class="button secondary" data-action="recruitment">前往星際招募 ↗</button></section>` : ''}
    ` : `
      <div class="roster-personnel"><div class="roster-personnel-art">${formImage(currentId ?? originalForm(character.id))}<span>${esc(character.english)}</span></div><div class="roster-personnel-copy"><span class="roster-personnel-state">${captain ? '★ 隊長 · ' : ''}${statusLabel(save, character.id)}</span><h3>${esc(character.name)}</h3><p>${esc(character.role)}</p><div class="roster-role-tags">${roleTags[character.id].map(tag => `<span>${esc(tag)}</span>`).join('')}</div>${current ? `${elementBadge(current.damageType)}<small>${esc(current.name)}</small>` : '<small>取得任一造型即可招募</small>'}<strong class="roster-outfit-count">造型 ${count} / ${forms.length}${count > 1 ? ' · 可換裝' : ''}</strong></div></div>
      <div class="roster-character-links"><button class="button secondary" data-action="roster-wardrobe">${count > 1 ? '造型換裝' : '查看造型'} · ${count} / ${forms.length} ↗</button><button class="button secondary" data-action="character" data-id="${character.id}">${deepTreesFor(character.id).length} 條技能樹 ↗</button></div>
