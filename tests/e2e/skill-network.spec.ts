...
for(const viewport of [{width:320,height:500},{width:390,height:844}])test(`complete mobile tree pans, zooms and retains camera at ${viewport.width}`,async({page},info)=>{
 ...
 const zoom=await map.getAttribute('data-scale');const target=page.locator('[data-action="personnel-skill-node"][data-id="C01-A3/4"]');await target.focus();await page.keyboard.press('Enter');
...
test('a pending merge keeps its other prerequisite and cross-route unlocks work in battle',async({page})=>{
 ...
 await page.evaluate(()=>{const s=window.__game.state()!;s.xp=180;s.choicesEarned=6;window.__game.ticks(1);for(const nodeId of ['C01-A3/0','C01-A3/1'])window.__game.command({type:'buy-node',offerId:s.draft!.id,nodeId});});
 await page.locator('[data-action="deep-owner"][data-id="C01"]').click();await node(page,'C01-A3/5');await node(page,'C01-A3/2');await node(page,'C01-A3/5');
 expect(await page.evaluate(()=>window.__game.state()!.draft!.pendingNodeIds)).toEqual(['C01-A3/2']);
 await node(page,'C01-B3/6');await expect(page.locator('[data-action="buy-node"]')).toBeEnabled();await page.locator('[data-action="buy-node"]').click();
 expect(await page.evaluate(()=>window.__game.state()!.treeNodes)).toEqual(['C01-A3/0','C01-A3/1','C01-A3/2','C01-B3/6']);
 await node(page,'C01-B3/3');await node(page,'C01-B3/7');await page.locator('[data-action="buy-node"]').click();
