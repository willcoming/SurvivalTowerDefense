...
test('ANIM PERF: 60 seconds of live 3× simulation and animation under synthetic peak density', async ({ page }, info) => {
  ...
  const fixture = await page.evaluate(async plan => {
    ...
    for (let i = 0; i < 12; i++) s.fields.push({ id: s.nextEntityId++, source: i % 2 ? 'C04' : 'C01', kind: i % 2 ? 'gravity' : 'fire', x: 65 + i % 3 * 120, y: 80 + Math.floor(i / 3) * 95, radius: 45, expires: 999999, nextTick: s.tick, dps: 1, damageType: 'thermal', slow: .1, slowDuration: 30, pull: 0, burnDuration: 30, armorIgnore: 0 });
    // Earned-points fixture, but every node is acquired through legal new-tree commands.
    ...
    s.xp=720;s.choicesEarned=24;openDraft(s);
    for(const nodeId of plan)if(!command(s,{type:'buy-node',offerId:s.draft!.id,nodeId}))throw new Error('Invalid pressure build');
    s.enemies.at(-1)!.chargeKind = 'boss'; s.enemies.at(-1)!.chargeUntil = 999999;

    return { autoTactical: true, enemies: 120, persistentHostileProjectiles: 400, fields: 12, independentSyntheticFieldSources: ['C04','C01'], evolutions: 3, bossId: 'B03', speed: 3, liveSimulation: true, movingTypes: ['E01','E02','E03','E04','E05','E06','E07','E08'], positionRecycling: true };
  }, buildPlan('C05-A/8',7).plan);
