...
test('AUTO: all six captains cast through normal gameplay; preference carries into a skill-disabled challenge',async({page},info)=>{
  ...
  for(const captainId of ['C01','C02','C03','C04','C05','C06'] as const){
    await page.evaluate(captainId=>window.__game.start({stageId:'S01',squadIds:[captainId],captainId,seed:101}),captainId);
  ...
  await page.evaluate(()=>window.__game.start({stageId:'S01',squadIds:['C06'],captainId:'C06',seed:101,challengeId:'no-skill'}));
